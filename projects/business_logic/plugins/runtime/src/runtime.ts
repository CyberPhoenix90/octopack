...
async function createBrowserImportMap(
	model: ProjectBuildData,
	workspaceRoot: string,
	outFilePath: string
): Promise<string> {
	const outDir = parse(outFilePath).dir;
	const result: string[] = [];
	for (const dep of model.project.projectDependencies) {
		let output = join(
	...
	for (const dep of model.project.fileDependencies.keys()) {
		...
		depPath = relative(
			outDir,
			join(model.project.path, model.project.resolvedConfig.build.bundles[model.bundle].output, depPath)
		);
