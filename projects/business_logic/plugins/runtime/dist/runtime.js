...
async function generateBrowserRuntime(model, context, runtime, path) {
    ...
    runtime.push('(() => {');
    runtime.push('\n');
    runtime.push(await createBrowserImportMap(model, context.workspaceRoot, path));
    runtime.push('\n');
...
async function createBrowserImportMap(model, workspaceRoot, outFilePath) {
    const outDir = path_1.parse(outFilePath).dir;
    const result = [];
    for (const dep of model.project.projectDependencies) {
    ...
    for (const dep of model.project.fileDependencies.keys()) {
        let depPath = model.project.fileDependencies.get(dep);
        depPath = path_1.relative(outDir, path_1.join(model.project.path, model.project.resolvedConfig.build.bundles[model.bundle].output, depPath));
