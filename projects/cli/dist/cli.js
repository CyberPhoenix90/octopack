...
(async () => {
	const { config, directory: firstDirectory } = await config_resolver_1.findConfiguration(
		process.cwd(),
		file_system_1.localDiskFileSystem
	);
	if (config === undefined) {
		notFound();
	}
	if (config.scope !== 'workspace') {
		const { config, directory } = await config_resolver_1.findConfiguration(
			path_1.join(firstDirectory, '..'),
			file_system_1.localDiskFileSystem
		);
		if (config === undefined) {
			notFound();
...
function runScript(config, workspaceRoot) {
	...
	const uiLogger = new logger_1.Logger({
		adapters: [
			...
			new logger_1.CallbackLoggerAdapter((log) => {
				var _a;
				return devLogger.log(((_a = log.text), _a !== null && _a !== void 0 ? _a : log.object), log.logLevel);
			})
		],
		enhancers: [new logger_1.LogLevelPrependerLoggerEnhancer()]
	});
