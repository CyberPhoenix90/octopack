...
class ProjectCrawler {
    ...
    async searchDirectory(result, path, fileSystem) {
        if (await fileSystem.exists(path_1.join(path, config_resolver_1.OCTOPACK_CONFIG_FILE_NAME))) {
            const config = await config_resolver_1.loadConfig(path, fileSystem);
